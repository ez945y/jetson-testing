...
| DEP-10 | **`docker buildx` 建置失敗 exit 125（實機回報，apt docker.io 的後果）** | 🔴 待實機確認 | 實機 `./build.sh` 在**第一個 `build-essential` 階段**就 `docker buildx build ... returned non-zero exit status 125`（base 還是乾淨的 `ubuntu:22.04`）→ **不是依賴問題，是 buildx/BuildKit 環境**。頭號嫌疑：DEP-9 為避開 no-curl 而用 `apt install docker.io`，**Ubuntu 的 `docker.io` 常不帶 buildx plugin / 較新 BuildKit**，而 jetson-containers 用 `docker buildx`。次要嫌疑：`/etc/docker/daemon.json` 的 `default-runtime` 非 `nvidia`。**診斷**：`tail -n 30 jetson-containers/logs/*/build/*build-essential.txt`（真錯誤在此）、`docker buildx version`、`cat /etc/docker/daemon.json`。**修法**：從 Docker 官方源補 `docker-buildx-plugin`，或 `nvidia-ctk runtime configure --set-as-default`（見 README 疑難排解）。**教訓**：`docker.io` 省事但不完整；jetson-containers 這種重度用 buildx 的工具，Docker 該從官方源裝。此為 DEP-9 那個「用 apt 省 curl」取捨浮現的代價。 |
