...
| DEP-14 | **官方 DS7.1 容器不內建 pyds（實測確認）** | 🟢 已修（待實機過關） | preflight 實測 12/13 過，唯一 FAIL = `import pyds`——先前論壇說 DS7.1 容器含 pyds 1.2.0 **是錯的**，官方容器沒帶。官方 wheel 在 [deepstream_python_apps releases](https://github.com/NVIDIA-AI-IOT/deepstream_python_apps/releases)：**v1.2.0 = DS 7.1 對應版、cp310 對容器 Python 3.10**（v1.2.2 是 cp312/DS8/JP7 用，不適用）。Dockerfile 層3 直接 pip 裝該 wheel + 補 `python3-gi`/`python3-gst-1.0`（app.py 的 `gi.repository` 需要，preflight 沒測到但 app 會用）。**另**：容器啟動 banner 警告 driver 540<560——Triton 對 dGPU 的檢查，Jetson iGPU 上 torch.cuda/deepstream-app 實測全過，屬噪音，暫記不理。 |
| DEP-15 | **模型權重 runtime 下載卡 DNS → 烤進 image** | 🟢 已修 | `./run.sh app` 實跑：pipeline/torch 都起來了，但 torchvision 首次下載 MobileNet 權重（download.pytorch.org）卡在 `getaddrinfo`。且容器 `--rm`，每次啟動都要重抓——設計本來就差。**修法**：Dockerfile 層4在 build 時下載一次權重進 image（`/root/.cache/torch`），runtime 零網路依賴。**同場加映（噪音清單，全部無害）**：Triton driver 560 banner（dGPU 檢查，iGPU 不適用）；`Authorization required`/`EGL failed`（無 X11 授權，headless 用不到）；Argus/nvargus-daemon（CSI 相機，我們走 v4l2）；gst 音訊外掛缺 FLAC/mp3 庫（與影像無關）。 |
